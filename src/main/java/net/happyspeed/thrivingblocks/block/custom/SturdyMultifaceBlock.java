...
public abstract class SturdyMultifaceBlock
        extends Block {
    ...
    public SturdyMultifaceBlock(AbstractBlock.Settings settings) {
        super(settings);
        this.setDefaultState(SturdyMultifaceBlock.withAllDirections(this.stateManager));
        this.SHAPES = this.getShapesForStates(SturdyMultifaceBlock::getShapeForState);
    ...
    public static Set<Direction> collectDirections(BlockState state) {
        if (!(state.getBlock() instanceof SturdyMultifaceBlock)) {
            return Set.of();
        }
        EnumSet<Direction> set = EnumSet.noneOf(Direction.class);
        for (Direction direction : Direction.values()) {
            if (!SturdyMultifaceBlock.hasDirection(state, direction)) continue;
            set.add(direction);
        }
        return set;
    }
    ...
    protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
        for (Direction direction : DIRECTIONS) {
            ...
            builder.add(SturdyMultifaceBlock.getProperty(direction));
        }
    }

    @Override
    public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState neighborState, WorldAccess world, BlockPos pos, BlockPos neighborPos) {
        if (!SturdyMultifaceBlock.hasAnyDirection(state)) {
            return Blocks.AIR.getDefaultState();
        }
        if (!SturdyMultifaceBlock.hasDirection(state, direction) || SturdyMultifaceBlock.canGrowOn(world, direction, neighborPos, neighborState)) {
            return state;
        }
        return SturdyMultifaceBlock.disableDirection(state, SturdyMultifaceBlock.getProperty(direction));
    }
    ...
    public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        ...
        for (Direction direction : DIRECTIONS) {
            ...
            if (!SturdyMultifaceBlock.canGrowOn(world, direction, blockPos, world.getBlockState(blockPos))) {
                return false;
            }
            bl = true;
        }
    ...
    public boolean canReplace(BlockState state, ItemPlacementContext context) {
        return SturdyMultifaceBlock.isNotFullBlock(state);
    }
    ...
    public BlockState withDirection(BlockState state, BlockView world, BlockPos pos, Direction direction) {
        ...
        return (BlockState)blockState.with(SturdyMultifaceBlock.getProperty(direction), true);
    }
    ...
    private BlockState mirror(BlockState state, Function<Direction, Direction> mirror) {
        ...
        for (Direction direction : DIRECTIONS) {
            if (!this.canHaveDirection(direction)) continue;
            blockState = (BlockState)blockState.with(SturdyMultifaceBlock.getProperty(mirror.apply(direction)), state.get(SturdyMultifaceBlock.getProperty(direction)));
        }
        return blockState;
    }

    public static boolean hasDirection(BlockState state, Direction direction) {
        BooleanProperty booleanProperty = SturdyMultifaceBlock.getProperty(direction);
    ...
    private static BlockState disableDirection(BlockState state, BooleanProperty direction) {
        ...
        if (SturdyMultifaceBlock.hasAnyDirection(blockState)) {
            return blockState;
        }
        return Blocks.AIR.getDefaultState();
    }
    ...
    protected static boolean hasAnyDirection(BlockState state) {
        return Arrays.stream(DIRECTIONS).anyMatch(direction -> SturdyMultifaceBlock.hasDirection(state, direction));
    }
